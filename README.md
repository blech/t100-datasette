...
2. **Run the Datasette server:**
   ...
   ```
   *Note: Using `.` as the target directory tells Datasette to load the databases, `metadata.yml`, and `settings.json` automatically.*
...
- **`metadata.yml`**: Contains Datasette configuration, including canned SQL queries (`routes`, `routes_map`, `route`, `pairs_drilldown`) which power the main drill-down views and map visualizations.
...
The `metadata.yml` file defines several powerful canned queries that you can access via the Datasette UI:
